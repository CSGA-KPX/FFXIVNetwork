...
let inline internal swap (left : 'a byref) (right : 'a byref) =
    let temp = left
    left <- right
    right <- temp

type FFXIVCrypto private() = 
    let  N = 16
    let  p = RawP()
    let s0 = RawS0()
    let s1 = RawS1()
    ...
    let oneRound(x : uint32) = 
        let xa = BitConverter.GetBytes(x)
        let xa = xa |> Array.map (fun b -> (int b))
        let (a, b, c, d) = xa.[3], xa.[2], xa.[1], xa.[0]

        let A = s0.[a]
        let B = s1.[b]
        let C = s2.[c]
        let D = s3.[d]
        let mutable y = s0.[a] + s1.[b]
        y <- y ^^^ (s2.[c])
        y <- y + s3.[d]
        y
    ...
    static let instance = new FFXIVCrypto()

    member x.Initialize(ticket : string, clientNum : uint32) =
        let rawKey = 
            use ms = new MemoryStream(0x2C)
            use br = new BinaryWriter(ms)
            br.Write(0x78uy)
            br.Write(0x56uy)
            br.Write(0x34uy)
            br.Write(0x12uy)
            ...
            br.Write(0x00uy)
            br.Write(0x00uy)
            let ticketBytes = Text.Encoding.ASCII.GetBytes(ticket)
            br.Write(ticketBytes, 0, ticketBytes.Length)
            let bytes = ms.GetBuffer()
            use md5 = MD5.Create()
            md5.ComputeHash(bytes)
        let kl = rawKey.Length
        for i = 0 to p.Length - 1 do 
            let b = 
                let kl = rawKey.Length
                let j = i * 4
                [| rawKey.[(j+0) % kl]; rawKey.[(j+1) % kl];
                   rawKey.[(j+2) % kl]; rawKey.[(j+3) % kl];|]
                |> Array.fold (fun acc byte -> (acc <<< 8) ||| (int (sbyte byte))) 0
            p.[i] <- p.[i] ^^^ (uint32 b)
            
        let mutable xl = 0x0u
        let mutable xr = 0x0u
        for i in 0 .. 2 .. N do 
            let (l, r) = encipher(xl, xr)
            //printfn "%A" (l, r)
            xl <- l
            xr <- r
            p.[i]     <- xl
    ...
    member x.IsReady() = ready

    member x.Encipher(data : byte []) = 
        if (data.Length % 8) <> 0 then
            failwith "Data length needs to be a multiple of 8"
        data
        |> Array.chunkBySize 8
        |> Array.map (fun bs ->
            let head = BitConverter.ToUInt32(bs.[0 .. 3] |> Array.rev, 0)
            let tail = BitConverter.ToUInt32(bs.[4 .. 7] |> Array.rev, 0)
            let (l, r) = encipher(head, tail)
            let head = BitConverter.GetBytes(l)
            let tail = BitConverter.GetBytes(r)
            [| yield! head; yield! tail |])
        |> Array.toSeq
        |> Array.concat

    member x.Decipher(data : byte []) = 
        if (data.Length % 8) <> 0 then
            failwith "Data length needs to be a multiple of 8"
        data
        |> Array.chunkBySize 8
        |> Array.map (fun bs ->
            let head = BitConverter.ToUInt32(bs.[0 .. 3], 0)
            let tail = BitConverter.ToUInt32(bs.[4 .. 7] , 0)
            let (l, r) = decipher(head, tail)
            let head = BitConverter.GetBytes(l)
            let tail = BitConverter.GetBytes(r)
            [| yield! head; yield! tail |])
        |> Array.toSeq
